...
fs.readFile("./housingData.csv", "utf8", function(error, data) {
  ...
  fs.readFile("./sa2.geojson", "utf8", function(error, data) {
    ...
    geoData.forEach(function(d) {
      ...
      } else if (matches.length == 1 && matches[0].type == "units") {
        let match = matches[0];
    ...
    fs.writeFile("./unitData.geojson", JSON.stringify(finalData), function(error) {
      console.log("unitData.geojson written");
    });
  });
});
